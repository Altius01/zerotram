...
public class Spawner : MonoBehaviour
{
    ...
    public void Spawn(GameObject spawnPoint, string passengerName = "", TicketAdditionMode ticketMode = TicketAdditionMode.Default)
    {
        ...
        else if (IsDefault(passengerName) || !IsOpened(passengerName) || IsOther(passengerName))
        {
        ...
        {
            DoRegularSpawn(spawnPoint);
            Debug.Log("Попался!" + ' ' + passengerName);
        }
    }
    ...
    private void DoRegularSpawn(GameObject spawnPoint)
    {
        ...
        for (int i = 0; i < realCount; i++)
        {
            ...
            while (IsOpened(passengerString) && !IsOther(passengerString) && !IsDefault(passengerString))
                passengerString = MapManager.GetInstance().GetRandomCharacter();
    ...
    public bool IsDefault(string name)
    {
        ...
        if (ConfigReader.GetConfig().GetField("map").GetField(MapManager.
            GetInstance().GetCurrentWorld()).GetField("Default passengers").HasField(name))
        {
            _isDefault = true;
        }
        else
    ...
    public bool IsOpened(string name)
    {
        ...
        if (EncryptedPlayerPrefs.GetString(name + "_isOpened") == "t")
        {
            _isOpened = true;
        }
        else
    ...
    public bool IsOther(string name)
    {
        bool _isOther;

        if (ConfigReader.GetConfig().GetField("map").GetField(MapManager.
            GetInstance().GetCurrentWorld()).GetField("Other passengers").HasField(name))
        {
            _isOther = true;
        }
        else
