...
public class PassengerFlyingAwayState : MovableCharacterState
{
    ...
    private Dictionary<GameController.ScrollsTypes, float> ScrollsProbabilities;
    ...
    public void DropBonus()
    {
        ...
        if (MonobehaviorHandler.GetMonobeharior()
                ...
                .IsAnyBonusActive())
            return;
        if (Randomizer.GetPercentageBasedBoolean((int)ConfigReader.GetConfig().GetField("map").GetField(MapManager.GetInstance().GetCurrentWorld()).GetField("ScrollsProps").n))
        {
            ParseScrollsTable();
            GameController.ScrollsTypes scrollType = Randomizer.CalculateValue<GameController.ScrollsTypes>(ScrollsProbabilities);
            Scroll scr = new Scroll();
            if (EncryptedPlayerPrefs.HasKey(scrollType.ToString() + "_isOpened"))
                return;
            switch (scrollType)
            {
                case GameController.ScrollsTypes.granny:
                    scr.SetName("granny");
                    break;
                case GameController.ScrollsTypes.bird:
                    scr.SetName("bird");
                    break;
                case GameController.ScrollsTypes.cat:
                    scr.SetName("cat");
                    break;
                default:
                    return;
    ...
    protected void ParseScrollsTable()
    {
        ScrollsProbabilities = new Dictionary<GameController.ScrollsTypes, float>();
        JSONObject unparsedMap = ConfigReader.GetConfig().GetField("ScrollsTable");
        foreach (var scroll in Enum.GetValues(typeof(GameController.ScrollsTypes)))
        {
            string representation = scroll.ToString();
            if (unparsedMap.HasField(representation))
            {
                ScrollsProbabilities.Add((GameController.ScrollsTypes)scroll, unparsedMap.GetField(representation).n);
            }
        }
    }
}
